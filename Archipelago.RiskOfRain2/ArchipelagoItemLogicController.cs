...
namespace Archipelago.RiskOfRain2
{
    public class ArchipelagoItemLogicController : IDisposable
    {
        ...
        public event Action<string> OnLootPoolExpansionReceived;
        public event Action OnSkillUnlockReceived;
        public event Action OnSurvivorUnlockReceived;
        ...
        private Queue<KeyValuePair<long, string>> customReceivedQueue = new Queue<KeyValuePair<long, string>>();
        ...
        private const long customRangeLower = 37600;
        private const long customRangeUpper = 37699;
...
/*                case ArchipelagoPacketType.ReceivedItems:
        ...
        public void EnqueueItem(long itemId)
        {
            ...
            //  when the run starts.
            if (customRangeLower <= itemId && itemId <= customRangeUpper)
            {
                customReceivedQueue.Enqueue(new KeyValuePair<long, string>(itemId, itemName));
            }
            else if (environmentRangeLower <= itemId && itemId <= environmentRangeUpper)
            {
        ...
        private void RoR2Application_Update(On.RoR2.RoR2Application.orig_Update orig, RoR2Application self)
        {
            ...
            if (customReceivedQueue.Any())
            {
                HandleReceivedCustomQueueItem();
            }
            if (IsInGame)
            {
        ...
        private void HandleReceivedCustomQueueItem()
        {
            KeyValuePair<long, string> itemReceived = customReceivedQueue.Dequeue();
            long itemId = itemReceived.Key;
            string itemName = itemReceived.Value;

            // Pool Expansion items: 37601-37606
            if (itemId >= 37601 && itemId <= 37606)
            {
                string[] tiers = { "White", "Green", "Red", "Boss", "Lunar", "Equipment" };
                string tier = tiers[itemId - 37601];
                Log.LogInfo($"Received Pool Expansion ({tier}): {itemName}");
                OnLootPoolExpansionReceived?.Invoke(tier);
            }
            // Skill Unlock: 37607
            else if (itemId == 37607)
            {
                Log.LogInfo($"Received Skill Unlock: {itemName}");
                OnSkillUnlockReceived?.Invoke();
            }
            // Survivor Unlock: 37608
            else if (itemId == 37608)
            {
                Log.LogInfo($"Received Survivor Unlock: {itemName}");
                OnSurvivorUnlockReceived?.Invoke();
            }
            else
            {
                Log.LogWarning($"Unknown custom item {itemId}: {itemName}");
            }
        }
